...
script_folder="$1"
repos_folder="/workspace/repos"
setup_installs="/workspace/configs/installs"
...
install-requires()
{
    ...
    if [[ -f "requirements.txt" ]]; then
       /workspace/configs/virenv/bin/python -m pip install -r requirements.txt
    fi
    ...
    for directory in ${repos_folder}/*; do
      if [ -d "$directory" ]; then
        if [[ -f "$directory/setup.py" ]]; then
          ...
          /workspace/configs/virenv/bin/python -m pip install -e $directory
        fi 
      fi
    done
